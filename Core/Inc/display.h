...
// 				- BldcControl
// 				- OledDisplay
// !!! Edit it only under BdlcControl project - this is the parent project for this file
// !!! Some code is blocked by PCB_VER-definition - it is available only for BldcContr

#define DISP_LIB_VER 2
...
typedef enum             // type of message (label in message)
{
  ...
  DISP_SYS_MSG_WRN,     // Blink. 		Message with warning-label.
  DISP_SYS_MSG_BIRD,    // Blink. 		Success (bird-label) message.
  DISP_SYS_MSG_NOMARK,  // NO Blink.	Message without marks
                        // message WITH buttons
  // 1 button
  DISP_SYS_MSG_WRN_OK,     // Blink. 		Message with warning-label.				OK button
  DISP_SYS_MSG_BIRD_OK,    // Blink. 		Success (bird-label) message.			OK button
  ...
  // 2 button
  DISP_SYS_MSG_WRN_NY,      // Blink. 		Message with warning-label.									NO-YES buttons
  DISP_SYS_MSG_WRN_OKINFO,  // Blink.			Message with warning-label.									OK-INFO buttons
  DISP_SYS_MSG_SETT,        // NO Blink. 	Message with arrows pointed in the center. 	BACK-OK buttons
  DISP_SYS_MSG_QUE,         // Blink. 		Question-label message. 										NO-YES buttons
} DISPSYSMSG;
...
typedef struct
{
  u8 txBuff[DISP_TX_BUFF];      // tx buffer for display
  u8 txPackNum[DISP_PAGE_NUM];  // amount of tx pack for display for each type of display page
  u8 txPackPnt;                 // current tx-pack point
  u8 pageIndex;                 // active page of display
  u16 pauseTmr;                 // pause after each transmitting
  u16 pauseTmrMax;              // max value of pause
...
typedef enum {
  ...
  DISP_PHD0_LIB_VER,                 // display library version
} DISP_RP_HELLO;
...
typedef enum {
  ...
  // display contrast
  DISP_PMD0_CONTRAST,
  // controller Uart timeout (useful for BootMode)
  DISP_PMD0_UARTTOUT,  // sec, !!! for all Pages
  // position adaptation
  DISP_PMD0_ADAPT_MIN,  // adaptation for minimum position was made
  DISP_PMD0_ADAPT_MAX,  // adaptation for maximum position was made
  // block A or B direction
  DISP_PMD0_BLOCK_A,
  DISP_PMD0_BLOCK_B,
} DISP_PMD0_MAIN;
...
// DISP_PMD0_COMM
// command bellow correspond to GATECOMM
...
// display contrast
#define DISP_PMD0_CONTRAST_LOW  50
#define DISP_PMD0_CONTRAST_HIGH 0xFF

#define DISP_PMS0_BTN_LEN       5   // length of buttons text
#define DISP_PMS0_BTN_NUM       5
#define DISP_PMS0_APN_LEN       15  // additional parameter name length
#define DISP_PMS0_APD_LEN       8   // additional parameter data length
...
typedef enum {
  ...
  DISP_PMS1_TOUT     = DISP_PMS1_NAME_C + DISP_PMS1_POS_LEN
} DISP_PMS1_MAIN;
